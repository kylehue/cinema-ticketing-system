...
public class ScheduleDateItem {
	...
	public ScheduleDateItem(String month, String day, String dayName) {
		this.month = month;
		this.day = day;
		...
		//Add click event on the date wrapper
		//When clicked, set to active.
		dateItemWrapper.setOnMouseClicked((MouseEvent event) -> {
			...
			for (int i = 0; i < children.size(); i++) {
				...
				ithChild.getStyleClass().remove("schedule-date-item-active");
			}
			
			//Make the clicked node active
			dateItemWrapper.getStyleClass().add("schedule-date-item-active");
		});
	}
